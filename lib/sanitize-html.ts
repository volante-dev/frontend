const allowedTags = new Set(["p", "br", "strong", "em", "b", "i", "ul", "ol", "li", "a", "h3", "h4"]);
  ...
  while ((match = tagPattern.exec(withoutComments))) {
    ...
    if (allowedTags.has(tag)) {
      if (isClosing && !voidTags.has(tag)) {
        ...
        const anchorAttributes = tag === "a" ? renderAnchorAttributes(attributes) : "";
        sanitized += voidTags.has(tag) ? `<${tag}>` : `<${tag}${anchorAttributes}>`;
      }
    }
